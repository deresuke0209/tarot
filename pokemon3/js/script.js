 ...
 //以下たたかうとポケモンのコマンドクリックによるウインドウの変化
  $(".battle-command").click(function(){
    if($(".monster0").hasClass("turn-rock")){//クリック重複処理を防ぐ
      return;
    }else if($(".battle-command").hasClass("rocked")){
      return;
    }else{
      $(".monster0-skils").show();
      $(".mymonter-list").hide();
...
function death_my_pokemon_change(){//自分のポケモンが瀕死になった際のチェンジ処理
  $('.mymonster0').find(".change-button").click(function(){//手持ち一匹目のポケモンに変える場合の処理
    if($(".monster0").hasClass("turn-rock")){//クリック重複処理を防ぐ
      ...
      setTimeout(function(){
        $(".battle-message0").text("コマンドを選んでください");
        $(".monster0").removeClass("turn-rock");
        $(".battle-command").removeClass("rocked")
      },4300);
...
}//death_my_pokemon_change(){//自分のポケモンが瀕死になった際のチェンジ処理

  function death_my_pokemon(){
    setTimeout(function(){
      $(".battle-message0").text('');
    },1000);
    ...
    setTimeout(function(){
      my_death_count ++;
      if(my_death_count == 3){//3匹全滅した場合
        alert("あなたの負けです");
        return;
      }//if(my_death_count == 3)3匹全滅した場合

      else{//まだ自分にポケモンが残っている場合
        setTimeout(function(){
          $(".change-message").text('次のポケモンを選んでください');
          $(".monster0").removeClass("turn-rock");
          $(".monster0-skils").hide();
          $(".mymonter-list").show();
          $(".battle-command").addClass("rocked");//ポケモンを選ぶまで技コマンドが開かないように

          death_my_pokemon_change();
        },2000);
